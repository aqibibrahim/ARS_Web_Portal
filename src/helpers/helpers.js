...
export const updateAmbulance = async (
	...
	try {
		const token = getToken()
		const response = await axios.patch(`https://ars.disruptwave.com/api/ambulances/${ambulanceId}`, updateData, {
			headers: {
				Authorization: `Bearer ${token}`,
			},
		})
...
export const updateHealthcare = async (
	...
	try {
		const token = getToken()
		const response = await axios.patch(`https://ars.disruptwave.com/api/facilities/${healthcareId}`, updateData, {
			headers: {
				Authorization: `Bearer ${token}`,
			},
		})
