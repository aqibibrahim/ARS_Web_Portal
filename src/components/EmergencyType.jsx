  ...
  const editGender = async () => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${Vars.domain}/emergency-types/${editIncidentID}`,
        data,
        {
          headers,
        }
      ...
      if (response.status === 200 || response.status === 201) {
        toast.success("تم تحديث نوع الطوارئ بنجاح", {
          className: "toast-align-right",
        });
        setIsLoading(false);
        setEditIncidentID("");
  ...
  const deleteGender = async () => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${Vars.domain}/emergency-types/${deleteIncidentID}`,
        {
          headers,
        }
      );
      if (response.status === 204 || response.status === 201) {
        toast.success("تم حذف نوع الطوارئ بنجاح", {
          className: "toast-align-right",
        });
        setIsLoading(false);
        setDeleteIncidentID("");
