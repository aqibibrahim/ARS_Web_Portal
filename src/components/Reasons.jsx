...
export default function Reasons() {
	...
	const handleChange = (event) => {
		...
		setValidationErrors({
			...validationErrors,
			[event.target.name]: event.target.value.trim() === '' ? `Reason Name is required` : '',
		})
	}
	...
	const editReason = async () => {
		...
		try {
			...
			const response = await axios.patch(`${Vars.domain}/reasons/${editIncidentID}`, data, {
				headers,
			})
			if (response.status === 200 || response.status === 201) {
	...
	const deleteReason = async () => {
		...
		try {
			...
			const response = await axios.delete(`${Vars.domain}/reasons/${deleteIncidentID}`, {
				headers,
			})
							...
							<div className="flex flex-row justify-between gap-4 mb-4">
								<div className="flex flex-col space-y-2 w-full">
									...
									{validationErrors.editreason && (
										<p className="text-red-500 text-sm text-right">{validationErrors.editreason}</p>
									)}
								</div>
							</div>
							{/* <FiDivideCircle /> */}
							...
							<tbody>
								{allReasons?.map((data, index) => (
									<tr key={index} className="hover:bg-white">
										...
										<td className="whitespace-nowrap px-3 py-4 text-md">{data?.reason}</td>
									</tr>
								))}
							</tbody>
						</table>
