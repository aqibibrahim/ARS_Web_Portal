...
export default function IncidentVIewModal(props) {
    ...
    >
      <div>
        ...
        <div className="p-5 text-right">
          ...
          <p className="font-bold">Created By: </p> <span>{showData?.created_by?.first_name+" "+showData?.created_by?.last_name}</span>
          <p className="font-bold">Created at: </p> <span>{formattedDate}</span>
          ...
          <p className={`bg-${getEmergencyTypeColor(showData?.emergency_type?.name)} text-white p-2 border rounded-full`}>
      {showData?.emergency_type?.name}
    </p>
          <p className="mt-0.5">
  ...
  showData?.ambulances?.map((ambulance, index) => (
    ...
    >
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {/* Equipment Details as Chips */}
        {/* <div className="flex flex-wrap gap-2 mt-4">
          {ambulance.equipments.map((equipmentDetails, eqIndex) => (
            <div
  ...
  {showData?.facilities?.length > 0 ? (
    showData?.facilities?.map((facility, index) => (
      <div
