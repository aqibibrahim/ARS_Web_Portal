...
import { BsArrowRightCircle, BsEye, BsSearch, BsPerson, BsPeople } from 'react-icons/bs'
...
export default function IncidentVIewModal(props) {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				const response = await axios.get(`https://ars.disruptwave.com/api/incidents/${IncidentID}`, {
					headers,
				})
	...
	const formattedDate = showData?.created_at ? new Date(showData.created_at).toLocaleString() : ''
			...
			<div className="" style={{ fontFamily: 'Cairo' }}>
				<div className="flex flex-row justify-between items-center mb-4 bg-grayBg-300 w-full p-5 overflow-hidden">
					...
					<div className="flex  justify-center w-full">
						<h3 className="text-2xl font-semibold text-center flex  ">تفاصيل الحادث</h3>
					</div>
            ...
            <span>{formattedDate}</span>
					...
					<div className="p-5 text-right w-1/2 text-lg">
						...
						<div className="flex justify-end text-base">
							...
							<div className="font-semibold ml-1 text-base"> : رقم الهاتف </div>{' '}
						</div>
					</div>
				</div>
				<div className=" border mt-2 border-gray-300 	" />
				<div className="p-5">
					...
					{showData?.ambulances?.length > 0 ? (
						showData?.ambulances?.map((ambulance, index) => (
							...
							<>
								{' '}
								<div className="flex flex-row justify-between p-5 text-base bg-gray-100 mt-4" key={ambulance.id}>
									<p className={` ${getStatusStyle(ambulance.status)}`}>{ambulance?.status}</p>
									{ambulance?.status === 'Dispatched' ? (
										<div>
											<p className=" w-full flex justify-end">
												<span className="text-green-400">
													{ambulance?.distance_info?.from_ambulance_to_incident?.rows[0]?.elements[0]?.distance?.text}
												</span>
											...
											<p>
												<span className="text-green-400">
													{' '}
													{ambulance?.distance_info?.from_ambulance_to_incident?.rows[0]?.elements[0]?.duration?.text}
												</span>
												<span className="font-semibold  ml-1">: الوقت المقدر</span>
											</p>
										</div>
				...
				{showData?.ambulances?.length > 0 ? (
					<>
						<div className=" border mt-2 border-gray-300 	" />
						<div className="p-5">
							<p className="text-xl mt-1 text-center font-semibold">تفاصيل السائق</p>
							{showData?.ambulances?.map((ambulance, index) => (
								// <div
								//   className="flex flex-col bg-gray-100 mb-5 mt-2 p-4 rounded"
								//   key={ambulance.id}
				...
				{showData?.ambulances?.length > 0 ? (
					<>
						<div className=" border mt-2 border-gray-300 	" />
						<div className="p-5">
							<p className="text-xl mt-1 text-center font-semibold">تفاصيل الرعاية الصحية </p>
							{showData?.ambulances?.map((ambulance, index) => (
								<div
									className="flex flex-row flex-wrap justify-between p-5 text-base bg-gray-100 mt-4"
									key={ambulance?.id}
									...
									{ambulance?.facility ? (
										<>
											...
											<div className="flex justify-between w-full">
												...
												<p className="items-center flex">
													<span className="mr-1"> {ambulance?.facility?.email}</span>
											...
											<div className="flex justify-center w-full m-auto text-xl font-semibold ">تفاصيل شخص مسؤول</div>
										...
										<div className="font-semibold w-full items-center justify-center flex">لم يتم تعيين المنشأة</div>
									)}
								</div>
							))}
						</div>
				...
				<div className="p-5  ">
					...
					<div className="flex flex-row justify-between p-5 text-base bg-gray-100  mt-4">
						<div className="flex justify-end">
							<span className="mr-1">{formatDateTime(showData?.created_by?.updated_at)}</span>{' '}
							<span className="font-semibold"> : تاريخ الانتهاء</span>
						</div>
						<div className="flex justify-end">
							<span className="mr-1">{formatDateTime(showData?.created_by?.created_at)}</span>{' '}
						...
						<div className="flex justify-end">
							<span className="text-green-500 mr-1">
								{showData?.created_by?.first_name + ' ' + showData?.created_by?.last_name}
							</span>{' '}
				...
				<p className="text-xl text-center font-bold mr-4 my-2 mb-4">موقع الحادث</p>
