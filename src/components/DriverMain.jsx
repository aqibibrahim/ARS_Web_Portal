...
import { BiCalendarMinus, BiEdit, BiMessageAltError, BiMessageAltX } from 'react-icons/bi'
...
export default function DriverMain() {
	...
	const handleNewPin = async () => {
		...
		if (isValid) {
			try {
				...
				const response = await axios.patch(`${Vars.domain}/drivers/update-pin/${updatePINId}`, data, {
					headers,
				})
	...
	return (
		<>
                  ...
                  Driver Last Name
									...
									<div className="flex justify-end mt-5 ">
										<Pagination
											...
											showTotal={(total, range) => `${range[0]}-${range[1]} of ${total} السائق`}
										/>
									</div>
								</>
							) : (
			...
			<Modal
				title={
					<div style={{ fontFamily: 'Cairo', padding: 10 }} className="flex justify-end">
			...
			<Modal
				title={
					<span className="flex justify-end p-5" style={{ fontFamily: 'Cairo' }}>
						تحديث دبوس
					</span>
				}
				open={pinModal}
					...
					<div style={{ fontFamily: 'Cairo', padding: 10 }}>
						...
						<div className="relative mt-2">
							<input
								...
								required
							/>
							{validationErrors.newPin && <span className="text-red-500 text-xs mt-1">{validationErrors.newPin}</span>}
							<div
								className="absolute inset-x-0 bottom-0 border-t border-gray-300 peer-focus:border-t-2 peer-focus:border-primary-100"
								aria-hidden="true"
							/>
