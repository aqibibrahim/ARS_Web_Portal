...
export default function Gender() {
	...
	const editGender = async () => {
		...
		try {
			...
			const response = await axios.patch(`${Vars.domain}/genders/${editIncidentID}`, data, {
				headers,
			})
			if (response.status === 200 || response.status === 201) {
	...
	const deleteGender = async () => {
		...
		try {
			...
			const response = await axios.delete(`${Vars.domain}/genders/${deleteIncidentID}`, {
				headers,
			})
				...
				<div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full">
					<div className="mt-5 mx-auto p-0 border w-[600px] shadow-lg rounded-md bg-white overflow-hidden h-auto mb-5">
						<div className="flex flex-row justify-between items-center mb-4 bg-grayBg-300 w-full  p-5 overflow-hidden">
							...
							/>
							<h3 className="text-xl font-semibold">اضافة جنس الحالة</h3>
						</div>
						<div className="p-5">
				...
				<div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full">
					<div className="mt-5 mx-auto p-0 border w-[600px] shadow-lg rounded-md bg-white overflow-hidden h-auto mb-5">
						<div className="flex flex-row justify-between items-center mb-4 bg-grayBg-300 w-full  p-5 overflow-hidden">
							...
							/>
							<h3 className="text-xl font-semibold"> تحديث اسم الجنس </h3>
						</div>
						<div className="p-5">
							<div className="flex flex-row justify-between gap-4 mb-4">
								<div className="flex flex-col space-y-2 w-full">
			...
			>
				...
				<div className="text-right flex-col bg-white rounded-b-lg p-2 flex justify-end items-right  ml-20  -mt-1">
					<div className="p-4 text-right  bg-gray-100 ">
						<h1 className="text-xl font-semibold m-2">الجنس</h1>
						<div>
							<button
