...
export default function VehicleMake() {
	...
	const editVehcileMake = async () => {
		...
		try {
			...
			const response = await axios.patch(`${Vars.domain}/vehicle-make/${editIncidentID}`, data, {
				headers,
			})
			if (response.status === 200 || response.status === 201) {
	...
	const deleteVehicleMake = async () => {
		...
		try {
			...
			const response = await axios.delete(`${Vars.domain}/vehicle-make/${deleteIncidentID}`, {
				headers,
			})
							...
							<div className="flex flex-row justify-between gap-4 mb-4">
								<div className="flex flex-col space-y-2 w-full">
									...
									{validationErrors.editVehicleMake && (
										<p className="text-red-500 text-sm text-right">{validationErrors.editVehicleMake}</p>
									)}
								</div>
							</div>
							{/* <FiDivideCircle /> */}
