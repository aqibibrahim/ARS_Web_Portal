...
const CreateIncidentSidebar = ({ onClose, data, selectmap }) => {
	...
	const [isAssignedAmbulancesVisible, setIsAssignedAmbulancesVisible] = useState(false)
	...
	const { selectedIncidentId, setSelectedIncidentId, resetIncidentState } = useAmbulanceContext()
	useEffect(() => {
	...
	const renderFormForTab = () => {
		...
		switch (activeTab) {
			...
			case 'الرعاية_الصحية':
				return <HealthCareForm onClick={handelclose} datatt={ambulanceData} onClose={onClose} />
			default:
				return null
		}
	}
	...
	return (
		<>
			<div className={` flex  overflow-hidden   `}>
				{isAssignedAmbulancesVisible && <AssignedAmbulances onClose={() => setIsAssignedAmbulancesVisible(false)} />}
				<div
				...
				>
					<div className="flex flex-col px-2 py-1  rounded-[12px] bg-white w-[450px] h-screen overflow-hidden">
						<div className="flex flex-row justify-between items-center p-2.5">
							<img src={closeIcon} alt="close" className="w-7 h-7 cursor-pointer" onClick={onClose} />
							<h1 className="text-2xl font-semi">{tabConfig[activeTab]}</h1>
						</div>
						<div className="mb-3">
							<Tabs activeTab={activeTab} onTabClick={setActiveTab} />
						</div>
						<div className="flex-1 overflow-y-auto custom-scroll-style ">{renderFormForTab()}</div>
					</div>
				</div>
			</div>
		</>
...
const Header = ({ handleIncidentNext, getlatitude, getmap, handleIncidentNextEdit }) => {
	var token = localStorage.getItem('token')
	const { selectedIncidentId, setSelectedIncidentId, resetIncidentState } = useAmbulanceContext()
	const [myData, setMyData] = useState([])
	...
	const createIncident = useFormik({
		...
		onSubmit: (values) => {
			...
			const createIncident = async () => {
				try {
					await axios.post(`${Vars.domain}/incidents`, JSON, config).then((res) => {
						if (res.status === 200 || res.status === 201) {
							...
							toast.error('Failed to create incident. Unexpected status code: ' + res.status)
					...
					setLoadingMessage(false)
					{
						e?.response?.data?.data?.informer_name && e.response.data.data.informer_name.length > 0
							? toast.error(e.response.data.data.informer_name[0])
							: null
					}

					{
						e?.response?.data?.data?.description && e.response.data.data.description.length > 0
		...
		geocoder.geocode({ location: newPosition }, (results, status) => {
			if (status === 'OK') {
				if (results[0]) {
					...
					createIncident.setFieldValue('informer_address', results[0].formatted_address)
				} else {
					setAddress('No address available')
				}
			} else {
			...
			{selectedIncidentId ? (
				<div>
					<div className="mb-5 mt-2">
						<div>
							<label htmlFor="informer_address" className=" text-sm  font-sm leading-6 text-gray-900 text-right">
								<button
									type="button"
					...
					<div className="mb-5">
						<div>
							<label htmlFor="informer_phone_numbers" className="block text-sm mr-2 leading-6 text-gray-900 text-right">
								رقم التواصل
							</label>
							<div className="relative mt-2">
								<InputMask
							...
							{({ open }) => (
								<>
									...
									<div className="relative mt-2">
										<Listbox.Button className="relative w-full h-8 cursor-default rounded-md bg-white py-1.5 pl-10 pr-3 text-right text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-primary-100 sm:text-sm sm:leading-6">
											<span className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-2">
												<ChevronUpDownIcon className="h-5 w-5 text-gray-400 transform rotate-180" aria-hidden="true" />
											</span>
								...
								<>
									...
									<div className="relative mt-2">
										<Listbox.Button className="relative w-full h-8 cursor-default rounded-md bg-white py-1.5 pl-10 pr-3 text-right text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-primary-100 sm:text-sm sm:leading-6">
											...
											<span className="block truncate">{selectedEmergencyOption.label}</span>
										</Listbox.Button>
