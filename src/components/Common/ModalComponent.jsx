...
const ModalComponent = (props) => {
	const { visible, handle, editData, updateDriver, editVisible, editBit } = props
	...
	const UpdateDriver = async (editData) => {
		try {
			if (validateForm()) {
				...
				const response = await axios.patch(`${Vars.domain}/drivers${editData?.id}`, data, {
					headers,
				})
						...
						<div className="  -left-[14.5rem] p-0 border w-2/4 shadow-lg rounded-md bg-white  h-auto mb-5 m-auto mt-10">
							<div className="flex flex-row justify-between items-center mb-4 bg-grayBg-300 w-full  p-5 overflow-hidden">
								...
								<h3 className="text-xl font-semibold">
									{editData && editData.name ? 'Update Driver' : 'Create Driver'}
								</h3>
							</div>
							<div className="p-5">
								<div className="flex flex-row justify-between gap-4 mb-4">
									<div className="flex flex-col space-y-2 w-full">
										<div>
											...
											{validationErrors.pin && <p className="text-red-500 text-sm">{validationErrors.pin}</p>}
										</div>
