...
const EditHealthCare = ({ openModal, datatt }) => {
	...
	const createAssignAmbulance = async () => {
		...
		try {
			const response = await axios.patch(`${Vars.domain}/ambulances/${ambulanceID}`, JSON, config)
				...
				showAssignAmbulance?.map((ambulance, index) => (
					...
					>
						<p className="text-right">
							Model: {ambulance?.model?.make?.name} {ambulance?.model?.name} {ambulance?.plate_no}
						</p>
						...
						{ambulance?.facility ? (
							<div key={ambulance?.id}>
								...
								<p>
									<span>Name: </span> {selectedFacilityOption?.name || ambulance?.facility?.name}
								</p>
								<p>
									<span>Address: </span> {selectedFacilityOption?.address || ambulance?.facility?.address}
								</p>
								<p>
									<span>Email: </span> {selectedFacilityOption?.email || ambulance?.facility?.email}
								</p>{' '}
								{/* <p>
							...
							disabledAmbulanceIDs.includes(ambulance?.id) && (
								<div key={ambulance?.id}>
									...
									<p>
										<span>Phone Number: </span>
										{ambulance
											? ambulance?.facility?.phone_numbers?.map((phone) => <span key={phone?.id}>{phone?.number}</span>)
											: selectedFacilityOption?.phone_numbers?.map((phone) => (
													<span key={phone?.id}>{phone?.number}</span>
											  ))}
									</p>
