...
import { BsArrowRightCircle, BsEye, BsSearch, BsEyeSlash } from 'react-icons/bs'
...
export default function RolesPermission() {
	...
	const handleCheckboxChange = (privilege_id, type) => {
		...
		setPermissionModalData((prevData) =>
			prevData.map((module) => {
				return module.privilege_id === privilege_id ? { ...module, [`${type}`]: !module[`${type}`] } : module
			})
		)
	}
	const handleAssignPermissions = async () => {
		...
		const uniquePrivilegeIds = [...new Set(permissionArray.map((module) => module.privilege_id))]

		// Creating the final structure
		const finalData = {
			privilege_id: uniquePrivilegeIds,
			is_view: uniquePrivilegeIds.map(
				(id) => permissionArray.find((module) => module.privilege_id === id)?.is_view || 0
			),
			is_add: uniquePrivilegeIds.map((id) => permissionArray.find((module) => module.privilege_id === id)?.is_add || 0),
			is_edit: uniquePrivilegeIds.map(
				(id) => permissionArray.find((module) => module.privilege_id === id)?.is_edit || 0
			),
			is_delete: uniquePrivilegeIds.map(
				(id) => permissionArray.find((module) => module.privilege_id === id)?.is_delete || 0
			),
		}
		...
		try {
			...
			const response = await axios.patch(`${Vars.domain}/role-permission/${editID}`, finalData, {
				headers,
			})
	...
	const createNewUser = async () => {
		try {
			...
			if (editFlag) {
				...
				response = await axios.patch(`${Vars.domain}/users/${editUserID}`, Editdata, {
					headers,
				})
			} else {
			...
			if (response.status === 200 || response.status === 201) {
				{
					editFlag ? toast.success('User Updated Successfully') : toast.success('User Created Successfully')
				}
				setPhoneNumbers([])
				...
				<div>
					...
					>
						...
						<div className="text-right flex-col bg-gray-100 rounded-lg p-2 flex justify-end items-right">
							<h1 className="text-xl font-semibold m-2 mt-3 text-center">الأدوار والأذونات</h1>
							<div className="flex justify-end mb-2">
								<Tab
									selected={activeTab === 'Role'}
									title="الأدوار"
									onClick={() => handleTabChange('Role')}
									className={`${activeTab === 'Role' ? 'bg-blue-500 text-white' : 'bg-white text-blue-500'}`}
								/>{' '}
								<Tab
									selected={activeTab === 'User'}
									title="المستخدمين"
									onClick={() => handleTabChange('User')}
									className={`${activeTab === 'User' ? 'bg-blue-500 text-white' : 'bg-white text-blue-500'}`}
								/>{' '}
							</div>
						</div>
								...
								<>
									<div className="flex justify-end flex-col">
										<div className="text-right flex-col  rounded-lg p-2 flex justify-end ">
											<h1 className="text-xl font-semibold ml-2 mt-3 ">الأدوار</h1>
										</div>
														...
														<tr key={index} className="hover:bg-white">
															<td className=" whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-0">
																<span className="flex items-center  gap-4">
																	...
																	<button
																		onClick={() => handleEditClick(data?.label, data?.value)}
																		className="text-primary-100 hover:text-indigo-900 border-2 rounded-lg border-primary-100 py-1 px-2"
																	>
																		<BiEdit />
																	</button>
																	<button
																		onClick={() => handlePermissionClick(data?.value)}
																		className="text-primary-100 hover:text-indigo-900 border-2 rounded-lg border-primary-100 py-1 px-2"
																	>
																		تعيين الأذونات
																	</button>
															...
															<td className="whitespace-nowrap px-3 py-4 text-sm">{data?.label}</td>
														</tr>
													))}
												</tbody>
											</table>
									...
									{/* Delete Role Modal */}
									<Modal
										title={<div className="flex p-5 justify-end">هل أنت متأكد من حذف هذا الدور؟</div>}
										open={deleteModal}
										onOk={handleDelete}
									...
									<Modal
										title={
											<span className="flex justify-end p-4" style={{ fontFamily: 'Cairo' }}>
												تعديل الدور
											</span>
										}
										open={editModal}
									...
									<Modal
										title={
											<div className="flex justify-end p-5 " style={{ fontFamily: 'Cairo' }}>
												تسجيل دور جديد
											</div>
										}
										open={isModalOpen}
									...
									<Modal
										...
										style={{ padding: 10 }}
									>
										<h2 className="text-center font-semibold text-lg mb-3 ">Assign Permissions</h2>
										{/* Rest of your modal content */}
										<div className="flex flex-col w-full p-5">
											{permissionModalData.map((module, index) => (
												<div key={module.privilege_id} className="flex flex-row   mb-2">
													<div className="mr-2 font-semibold">{module.moduleName}:</div>
													<div className="ml-auto">
														<label className="mr-2 ">
															<input
																...
																onChange={() => handleCheckboxChange(module.privilege_id, 'isView')}
															/>
															<span> View</span>
														</label>
														<label className="mr-2">
															<input
																...
																onChange={() => handleCheckboxChange(module.privilege_id, 'isAdd')}
															/>
															<span> Add </span>
														</label>
														<label className="mr-2">
															<input
																...
																onChange={() => handleCheckboxChange(module.privilege_id, 'isEdit')}
															/>
															<span> Edit </span>
														</label>
														<label>
															<input
																...
																onChange={() => handleCheckboxChange(module.privilege_id, 'isDelete')}
															/>
															<span> Delete</span>
														</label>
													</div>
						...
						<div className="m-auto bg-white ">
							{activeTab === 'User' && (
								<div className="flex justify-end -mt-2 flex-col">
									<div className="text-right flex-col  rounded-lg p-2 flex justify-end ">
										{' '}
										<h1 className="text-xl font-semibold m-2 mt-3"> المستخدمين</h1>
														...
														<tbody>
															{allUsers?.map((data, index) => (
																<tr key={index} className="hover:bg-white">
																	...
																	<td className="whitespace-nowrap px-3 py-4 text-xs">{data?.email}</td>
																	<td className="whitespace-nowrap px-3 py-4 text-xs">{data?.first_name}</td>
																	<td className="whitespace-nowrap px-3 py-4 text-xs">{data?.last_name}</td>
																	<td className="whitespace-nowrap px-3 py-4 text-xs">{data?.designation}</td>
																	...
																	<td className="whitespace-nowrap px-3 py-4 text-xs">{data?.role?.name}</td>{' '}
																</tr>
															))}
														</tbody>
													</table>
