	...
	const createNewUser = async () => {
		try {
			...
			if (editFlag) {
				...
				response = await axios.patch(`${Vars.domain}/users/${editUserID}`, data, {
					headers,
				})
			} else {
			...
			const errorMessage = error.response?.data?.data?.email || 'An error occurred'
				...
				<Tabs defaultActiveKey="1" centered>
					<TabPane tab="Roles & Permissions" key="1">
						...
						>
							{' '}
							<div className="text-right flex-col bg-white rounded-lg p-2 flex justify-end items-right">
								<h1 className="text-2xl font-semibold m-2 mt-3"> Roles & Permissions</h1>
								<div>
									<button
										onClick={() => {
											NewRole()
						...
						>
							<div className="flex flex-col w-full">
								{permissionModalData.map((module, index) => (
									<div key={module.privilege_id} className="flex items-center mb-2">
										<span className="mr-2 font-semibold">{module.moduleName}:</span>
									...
									<tbody>
										{allUsers?.map((data, index) => (
											<tr key={index} className="hover:bg-white">
												...
												<td className="whitespace-nowrap px-3 py-4 text-md">{data?.first_name}</td>
												<td className="whitespace-nowrap px-3 py-4 text-md">{data?.last_name}</td>
												<td className="whitespace-nowrap px-3 py-4 text-md">{data?.designation}</td>
												...
												<td className="whitespace-nowrap px-3 py-4 text-md">{data?.role?.name}</td>
											</tr>
										))}
									</tbody>
								</table>
						...
						>
							<div className="p-5">
								...
								{phoneNumbers?.length > 0 ? (
									<div className={`grid grid-cols-2 gap-2 ${phoneNumbers?.length > 0 ? 'bg-gray-100' : ''} p-4`}>
										{phoneNumbers.map((phoneNumber, index) => (
											<div key={index} className="flex items-center justify-between text-lg bg-white p-2 rounded-md">
												<div className="flex text-sm">{phoneNumber?.number || phoneNumber}</div>
												<button
													type="button"
