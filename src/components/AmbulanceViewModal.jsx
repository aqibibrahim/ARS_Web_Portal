...
const AmbulanceViewModal = ({ viewOpen, selectedAmbulance, setViewOpen }) => {
	console.log('View open', viewOpen, 'Ambulance Data', selectedAmbulance, 'Set view open', setViewOpen)
		...
		>
			<div className="">
				<div className="flex flex-row justify-between items-center mb-4 bg-grayBg-300 w-full p-5 overflow-hidden z-10">
					...
					/>
					<h3 className="text-xl font-semibold text-center w-full">تفاصيل سيارة الإسعاف</h3>
				</div>
				<div>
					<div className="justify-between flex p-2 -mt-3 w-full">
						<div className="m-auto items-center justify-center text-xl w-1/2">
							<span className={` ${getStatusStyle(ambulanceData?.status)}`}>{ambulanceData?.status}</span>
						</div>
                    ...
                    Driver Phone No.
									...
									{ambulanceData?.driver?.phone_numbers.map((phone_numbers) => (
										<span key={phone_numbers.id} className="text-base text-right">
											<span className="flex bg-green-200 m-1 rounded-lg p-1"> {phone_numbers.number}</span>
										</span>
									))}
									<span className="font-semibold text-base  mt-2">: رقم الهاتف</span>
								</div>
							</div>
						) : (
							<div>
								<p className="text-base text-center bg-gray-200">لم يتم تعيين سائق حتى الآن</p>
							</div>
						)}
					...
					{ambulanceData?.equipments?.length > 0 ? (
						<>
							...
							<div className="px-1 mt-3 mb-2 ">
								<p className="text-lg text-center font-semibold">تفاصيل المعدات</p>
					...
					<div className=" mt-2 mb-2 border border-gray-300 	" />
					<p className="text-lg text-center font-semibold mb-3">موقع سيارة الإسعاف</p>
