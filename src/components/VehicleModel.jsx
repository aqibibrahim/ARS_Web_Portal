...
export default function VehicleModal() {
	...
	const handleChange = (event) => {
		...
		setValidationErrors({
			...validationErrors,
			[event.target.name]: event.target.value.trim() === '' ? `Vehicle Make is required` : '',
		})
	}
	...
	const editVehcileMake = async () => {
		...
		try {
			...
			const response = await axios.patch(`${Vars.domain}/vehicle-models/${editIncidentID}`, data, {
				headers,
			})
			if (response.status === 200 || response.status === 201) {
	...
	const deleteVehicleModel = async () => {
		...
		try {
			...
			const response = await axios.delete(`${Vars.domain}/vehicle-models/${deleteIncidentID}`, {
				headers,
			})
							...
							<div className="flex flex-row justify-between gap-4 mb-4">
								<div className="flex flex-col space-y-2 w-full">
									...
									{validationErrors.editVehicleModel && (
										<p className="text-red-500 text-sm text-right">{validationErrors.editVehicleModel}</p>
									)}
								</div>
							</div>
							{/* <FiDivideCircle /> */}
					...
					{isLoading ? (
                      ...
                      >
                        <BsEye />
										...
										<td className="whitespace-nowrap px-3 py-4 text-md">{data?.make?.name}</td>{' '}
